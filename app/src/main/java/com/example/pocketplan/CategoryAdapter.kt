...
class CategoryAdapter(
    ...
    override fun onBindViewHolder(holder: CategoryViewHolder, position: Int) {
        ...
        holder.categoryName.text = category        // Get goal from SharedPreferences
        val prefs = context.getSharedPreferences("GoalPrefs", Context.MODE_PRIVATE)
        val goalAmount = prefs.getString(category, "0") ?: "0"
        holder.categoryGoalAmount.text = "Goal: ${CurrencyManager.formatCurrency(context, goalAmount)}"
        ...
        holder.categorySpent.text = "-${CurrencyManager.formatCurrency(context, spent.toDouble())}"
        holder.categoryBalance.text = CurrencyManager.formatCurrency(context, balance.toDouble())
